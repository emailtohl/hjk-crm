...
public class OrganizationServiceImpl extends StandardService<Organization, Long> implements OrganizationService {
	...
	@Override
	public List<RevTuple<Organization>> getRevisions(Long id) {
		return organizationRepo.getRevisions(id).stream().map(rt -> {
			return new RevTuple<Organization>(toTransient(rt.entity), rt.defaultRevisionEntity, rt.revisionType);
		}).collect(Collectors.toList());
	}
